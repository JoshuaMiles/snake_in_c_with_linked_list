...
void draw_int(unsigned char top_left_x, unsigned char top_left_y, int integer) {
    int char_width = 5;
    int char_height = 8;

    if (integer > 9) {
        int ones_place = integer % 10;
        int tens_place = (int) integer / 10;
        for (unsigned char i = 0; i < char_width + char_width; i++) {
            for (unsigned char j = 0; j < char_height; j++) {
                set_pixel(top_left_x + i, top_left_y + j,
                          (pgm_read_byte(&(ASCII[ones_place + 15][i])) & (1 << j)) >> j);
            }
        }
        for (unsigned char i = 0; i < char_width; i++) {
            for (unsigned char j = 0; j < char_height; j++) {
                set_pixel(top_left_x + i, top_left_y + j,
                          (pgm_read_byte(&(ASCII[tens_place + 16][i])) & (1 << j)) >> j);
            }
        }
        ...
        for (unsigned char i = 0; i < char_width + char_width; i++) {
            for (unsigned char j = 0; j < char_height; j++) {
                set_pixel(top_left_x + i, top_left_y + j,
                          (pgm_read_byte(&(ASCII[integer + 16][i])) & (1 << j)) >> j);
            }
        }
    }
}
